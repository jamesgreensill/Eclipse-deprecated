...
public class ImGuiDriver : Singleton<ImGuiDriver>, IDisposable
{
    ...
    internal override void ApplicationTick()
    {
        ...
        io.DeltaTime = TimeDriver.Instance.DeltaTime;
    ...
    internal override void ApplicationDraw()
    {
        ImGui.Render();
    ...
    public void Dispose()
    {
        ...
        Raylib.UnloadTexture(m_FontTexture);
    }
    ...
    {
        ImGuiIOPtr io = ImGui.GetIO();
        io.DisplaySize = new Vector2(width, height) / m_ScaleFactor;
    }
    ...
    private unsafe void LoadFontTexture()
    {
        ...
        m_FontTexture = Raylib.LoadTextureFromImage(image);

        // Store texture id in imgui font
        io.Fonts.SetTexID(new IntPtr(m_FontTexture.id));
    ...
    private void DrawTriangles(uint count, int idxOffset, int vtxOffset, ImVector<ushort> idxBuffer,
        ImPtrVector<ImDrawVertPtr> idxVert, IntPtr textureId)
    {
        ushort index = 0;
